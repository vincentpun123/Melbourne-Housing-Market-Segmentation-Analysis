...
```{r Melbourne Housing Data (Subset) - Complete and 9 variables}
#Read Data----
...
Look for anomalies/outliers 
Prepare the data for input to cluster analysis
```{r Missing Values}
#No Missing Values ----
...
```{r Remove Extreme Outliers - Skewness, Kurtosis, Boxlplot, Filters}
...
```{r Will be reattached to cluster profiles}
...
```{r Visualize BuildingArea - Remove Outliers}
#PARAMETERS 
par(mfrow = c(2,3))
...
Interpretation is more difficult with rescaled variables 

By normalizing/scaling the data, we can avoid the analysis from being dominated by a small number of variables (most notably Price, Landsize) due to the disproportional scales.

At conclusion of analysis, take the inverse transformation so variables are back in their original units 
...
```{r WSS based on 3 clusters, rsquare for hierarchical cluster method}
####################################
...
***Evaluate Model***
Justify decisions regarding the number of clusters to retain
Select a single clustering solution to present to the management client.
Justify your selection

```{r GOODNESS OF FIT RESULTS}
#kmeans
#r square =  0.5362854
...
```{r Clustering solution vs Actual Type of Housing}
...
Determine the number of properties/houses in each group/cluster broken down by location (using either suburb names or latitude and longitude).

```{r Number of properties/houses in each group/cluster broken down by location (suburb or lat/long)}
